...
function generateFiles(projectInfo) {
    ...
    // 生成rider-config.js
    file = path.resolve(projectInfo.dir, 'edp-rider-config.js');
    scaffold.generate('spa/rider-config', file);
    ...
    // 更新module.conf
    // 添加默认的combine选项
    file = path.resolve(projectInfo.dir, 'module.conf');
    var moduleConfig = require('edp-core').util.readJSONFile(file);
