...
function build4Node(options, config) {
    ...
    function handleRouteInfo() {
        var name = info.route;
        var file = path.resolve(BASE_DIR, name);

        log.info('handle route file ...');

        if (!fs.existsSync(file)) {
            return Deferred.rejected(file + ' not exists ');
        }

        file = fs.readFileSync(file, 'utf8');
        file = file.replace(/action\s*:\s*([^,}\s]+)/g, function ($0, $1) {
            $1 = $1.trim();
            if ($1.charAt(0) === '\'' || $1.charAt(0) === '"') {
                $1 = 'require(\'./' + $1.replace(/["']/g, '') + '\')';
            }

            return 'action: ' + $1;
        });
        fs.writeFileSync(path.resolve(output, name), file);
